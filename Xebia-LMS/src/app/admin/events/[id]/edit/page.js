...
import { ArrowLeft, Save, Globe, Info, ShieldAlert } from "lucide-react";
...
export default function EditEventPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setLoading(true);
      try {
        // 1. Fetch user & verify permissions
        ...
        setBannerImage(fetchedEvent.bannerImage || "");
        setCategory(fetchedEvent.category || "Technical");
        setDepartment(fetchedEvent.department || "Engineering");
        setOrganizer(fetchedEvent.organizer || "Xebia Academy");
        setVenue(fetchedEvent.venue || "");
        setLocation(fetchedEvent.location || "");
        setStartDate(fetchedEvent.startDate ? fetchedEvent.startDate.slice(0, 16) : "");
        setEndDate(fetchedEvent.endDate ? fetchedEvent.endDate.slice(0, 16) : "");
        setRegistrationDeadline(fetchedEvent.registrationDeadline ? fetchedEvent.registrationDeadline.slice(0, 16) : "");
        setMaximumCapacity(fetchedEvent.maximumCapacity || 50);
        setCurrentRegistrations(fetchedEvent.currentRegistrations || 0);
        setEligibility(fetchedEvent.eligibility || "All Consultants");
        setSelectedBatches(fetchedEvent.batchRestrictions || []);
        setMode(fetchedEvent.mode || "ONLINE");
        setMeetingLink(fetchedEvent.meetingLink || "");
        setCourseRestrictions(fetchedEvent.courseRestrictions ? fetchedEvent.courseRestrictions.join(", ") : "");
        setStatus(fetchedEvent.status || "DRAFT");
        setCreatedBy(fetchedEvent.createdBy || "admin");
  ...
  const handleFormSubmit = async (e, updateStatus = null) => {
    e.preventDefault();
    ...
    try {
      ...
      toast.addToast("Event details updated successfully.", "success");
      router.push("/admin/events");
    } catch (err) {
      toast.addToast(err.message || "Failed to update event.", "error");
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-foreground">Edit Event Details</h1>
            <p className="text-xs text-text-muted">Modify schedule dates, capacity limits, or description for event ID: {id}</p>
          </div>
          <span className="px-3 py-1 bg-amber-50 text-amber-800 border border-amber-200 text-xs font-bold rounded-lg capitalize">
            ...
            <form className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-border/60">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    placeholder="DevOps Consultants only, Open for all"
                    value={eligibility}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-border/60">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-foreground">Event Status <span className="text-rose-500">*</span></label>
                  <select
                    value={status}
